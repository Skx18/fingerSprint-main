...
public class fingerPrintController {
        ...
        try {
            ...
            if (response.getStatusCode() == HttpStatus.CREATED) {
                ...
                return ResponseEntity.status(response.getStatusCode())
                        .body(Map.of("error", "No se pudo crear el usuario en la API externa"));
            }
    ...
    public List<FingerprintVerificationRequest.UserFingerprint> getStoredFingerprintsFromFlask() {
        String flaskUrl = "https://bomberos-flask.onrender.com/attendance/fingerPrint"; // Asegúrate de que Flask esté
                                                                                        // corriendo en este puerto
        RestTemplate restTemplate = new RestTemplate();
    ...
    public ResponseEntity<Map<String, Object>> verifyFingerprint() {
        try {
            ...
            if (users == null || users.length == 0) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(Map.of("message", "No se recibieron huellas."));
            }
            ...
            if (readers.isEmpty()) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(Map.of("message", "No se encontró ningún lector."));
            }
            ...
            if (result == null || result.image == null) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(Map.of("message", "No se pudo capturar la huella."));
            }
            ...
            ResponseEntity<String> flaskResponse = restTemplate.postForEntity(
                    "https://bomberos-flask.onrender.com/attendance/fingerPrint",
                    entity,
                    String.class);
